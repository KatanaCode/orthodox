...
RSpec.describe TfaSession, type: :model do
  ...
  before do
    member.create_otp_credential!
  end
  
  describe "#valid?" do
    
    subject { TfaSession.new(record: member) }
    
    context "when otp correct" do
        
      before do
        subject.otp = member.otp_credential.send(:current_otp)
      end
    ...
    context "when recovery code correct" do
      ...
      before do
        subject.recovery_code = member.otp_credential.recovery_codes.sample
      end
